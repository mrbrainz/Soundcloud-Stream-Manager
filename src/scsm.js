...
murkconsole = '<div id="murkconsole" style="position:absolute;top:-16px;left:0px; padding: 10px; border: 2px solid orange;z-index:999999;text-align: left;background: #fff;width: 100%;height:0px;overflow-y:auto;box-sizing: border-box;">\
        <div style="background:url(\''+nsscBGIMG+'\') top right no-repeat #fff;">\
        <h2 style="margin-bottom: 15px;">Stream Manager <span style="float: right; display:block;padding:2px 5px;border:1px solid #444;background:#efefef;cursor:pointer;font-size:12px;" onclick="murkCloseGraceful();return false;">X</span> <span style="float: right; display:block;padding:2px 5px;border:1px solid #444;background:#efefef;cursor:pointer;font-size:12px;margin-right: 5px;" onclick="murkMinimize(this);return false;">_</span> </h2>\
...
deadoutconsole = '<div id="murkconsole" style="position:fixed;top:10px;right:10px; padding: 10px; border: 2px solid orange;z-index:999999;text-align: left;background: #fff;min-width: 50px;">\
    <h2 style="margin-bottom: 15px;font-size: 20px;">This isn\'t your SoundCloud stream, silly...</h2>\
</div>',
...
function fixMurk() {
    ...
    if (windowpos  +56 >= pos.top) {
        s.css({'position':'fixed','left':spos.left,'top':56,'width':sw});
    } else {
    ...
    if ((window.location.href === 'https://soundcloud.com/stream') || (window.location.href === 'https://soundcloud.com/stream/') || window.location.href.includes("sclayouttest.html") ) {
         jQuery(document).ready(function() { 
            ...
            if (!jQuery('#murkconsole').length) {
                ...
                jQuery('.l-sidebar-right').animate({'paddingTop':murkConsoleHeight},500).prepend(murkconsole);
        ...
        jQuery('body').append(deadoutconsole);
        murkCloseGraceful();
    }
}
